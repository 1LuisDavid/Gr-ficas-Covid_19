...
covid %>% 
  ...
  labs(y="Número de casos reportados por día", x="Fecha",
       ...
       subtitle = "En países latinoamericanos con corte a las 6:00 am del 23 de marzo.",
...
####tratar de hacer una columna que indique el día 1 y subsecuentes de cada país-----

# df2
# write_csv(df2, "df3.csv")
# df3_1 <- read_csv("df3-03-22.csv")
# View(df3_1)
unique(df$País)
#---------------- MÉXICO -----------
df %>% 
...
diasmx$fecha <- c(seq(as.Date("2020-02-29"), as.Date("2020-03-23"), 1))
...
df3 %>%
  ...
  geom_label( x= 14, y=785, label="Chile cierra cines, bares, eventos deportivos, restaurantes.", size=3, color="#7CAE00")+
  geom_label( x= 16, y=930, label="Chile implementa toque de queda de 10pm a 5am. \nTambién, declaran cuarentena total en Puerto Williams.", size=3, color="#7CAE00")+
  ...
  geom_label( x= 23, y=1250, label="Brazil cierra fronteras a extranjeros.", size=3, color="#F8766D")+
  geom_label( x= 25, y=1400, label="Gobernador de Sao Paulo decreta cuarentena \nde 15 días apartir del martes 24 de marzo.", size=3, color="#F8766D")+
  geom_label( x= 19, y=1100, label="México suspende actividades en escuelas y \nanuncia suspensión de actividades no esenciales en \nlas oficinas de Administración Pública.", size=3, color="#C77CFF")+
  ...
  labs(x="Días transcurridos desde el primer caso reportado en cada país", y="Número acumulado de casos",
       title="Principales medidas contra el Covid-19 adoptadas por países latinoamericanos con corte al 23 de marzo",
       ...
       caption = "Fuente: Datos de European Center for Disease Control and Prevention (ECDC), publicados a la 1pm CET del 23/03/2020 || Noticias de El espectador.com || Gráfica elaborada por @1LuisDavid.")+
